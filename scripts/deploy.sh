...
# check FOREIGN_ORACLE_ADDRESS exists
if [ -z ${FOREIGN_ORACLE_ADDRESS+x} ]; then
    echo "FOREIGN_ORACLE_ADDRESS is not setted";
    exit 1
fi
...
# Create proxy
echo "> zos create Gateway --init initialize --args $OWNER_ADDRESS,$ORACLE_ADDRESS,$FOREIGN_ORACLE_ADDRESS --network deploy --from $ADMIN_ADDRESS"
./node_modules/.bin/zos create Gateway --init initialize --args $OWNER_ADDRESS,$ORACLE_ADDRESS,$FOREIGN_ORACLE_ADDRESS --network deploy --from $ADMIN_ADDRESS
